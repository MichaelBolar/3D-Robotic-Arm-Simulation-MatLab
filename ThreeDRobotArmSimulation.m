%Define the RigidBodyTree
rbt = rigidBodyTree('DataFormat','column','MaxNumBodies',6);

%Define the rigid bodies you want to use
body1 = rigidBody('body1');
body2 = rigidBody('body2');
body3 = rigidBody('body3');
body4 = rigidBody('body4');
body5 = rigidBody('body5');
body6 = rigidBody('body6');

home_position = 0

%Define the joints and the joint type for each body
jnt1 = rigidBodyJoint('jnt1','revolute');

display(trvec2tform([0.0, 0.1, 0.1]))

setFixedTransform(jnt1,trvec2tform([0.0, 0.0, 0.1]))
jnt1.HomePosition = home_position

%Do the same thing for each joint
jnt2 = rigidBodyJoint('jnt2','revolute');

setFixedTransform(jnt2,trvec2tform([0.0, 0.1, 0.15]))
%jnt2.PositionLimits = [-pi, pi]
jnt2.HomePosition = home_position

%Joint 3
jnt3 = rigidBodyJoint('jnt3','revolute');


setFixedTransform(jnt3,trvec2tform([0.0, 0.1, 0.1]))
jnt3.HomePosition = home_position


%Joint 4
jnt4 = rigidBodyJoint('jnt4','revolute');


setFixedTransform(jnt4,trvec2tform([0.0, 0.15, 0.1]))
jnt4.HomePosition = home_position

%Joint 5
jnt5 = rigidBodyJoint('jnt5','revolute');


setFixedTransform(jnt5,trvec2tform([0.0, 0.1, -0.01]))
jnt5.HomePosition = home_position

%Joint 6
jnt6 = rigidBodyJoint('jnt6', 'revolute');


setFixedTransform(jnt6,trvec2tform([0.0, 0.1, -0.1]))
jnt6.HomePosition = home_position


%Define an end effector
endeffector = rigidBody('endeffector');


endeffector.addFrame('toolTip','endeffector', trvec2tform([0.0, 0.1, 0.0]));

endeffectorjnt = rigidBodyJoint('endeffectorjnt','fixed');

%Access the rigid bodies joint properties and 
%assign each one its own joint property
body1.Joint = jnt1;
body2.Joint = jnt2;
body3.Joint = jnt3;
body4.Joint = jnt4;
body5.Joint = jnt5;
body6.Joint = jnt6;
endeffector.Joint = endeffectorjnt;

%Add each body in sequence
addBody(rbt,body1,'base');
addBody(rbt,body2,'body1');
addBody(rbt,body3,'body2');
addBody(rbt,body4,'body3');
addBody(rbt,body5,'body4');
addBody(rbt,body6,'body5');
addBody(rbt,endeffector,'body6');


display(homeConfiguration(rbt))
showdetails(rbt)
show(rbt)

%{
%Create a circle that can be traced
t = (0:0.2:10)'; %Time
count = length(t);
center = [0 0.4 0.3];
radius = 0.1;
theta = t*(2*pi/t(end));
points = center + radius*[cos(theta) sin(theta) zeros(size(theta))];

q0 = homeConfiguration(rbt);
ndof = length(q0);
qs = zeros(count, ndof);

ik = inverseKinematics('RigidBodyTree', rbt);
weights = [0, 0, 0, 1, 1, 0];
endEffector = 'toolTip'

qInitial = q0; % Use home configuration as the initial guess
for i = 1:count
    % Solve for the configuration satisfying the desired end effector
    % position
    point = points(i,:);
    qSol = ik(endEffector,trvec2tform(point),weights,qInitial);
    % Store the configuration
    qs(i,:) = qSol;
    % Start from prior solution
    qInitial = qSol;
end


figure
show(rbt,qs(1,:)');
view([45 30])  % Better 3D view angle
ax = gca;
ax.Projection = 'orthographic';
hold on
plot3(points(:,1),points(:,2),points(:,3))  % Use plot3 for 3D
axis equal
grid on


framesPerSecond = 15;
r = rateControl(framesPerSecond);
for i = 1:count
    show(rbt,qs(i,:)','PreservePlot',false);
    drawnow
    waitfor(r);
end
%}